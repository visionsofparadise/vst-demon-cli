...
- **`vst-demon-linux-x64.tar.gz`** (x86_64). Extract with `tar xzf vst-demon-linux-x64.tar.gz` — the tarball preserves the executable bit, so the extracted `vst-demon` is ready to run. Move it onto your `PATH` if you like (e.g. `~/.local/bin/`).
...
```
vst-demon --plugin ~/.vst3/"Surge XT.vst3" --preset ~/presets/my-patch.vstpreset
```
